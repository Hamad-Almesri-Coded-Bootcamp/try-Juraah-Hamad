...
export function pendingDangerGuidance(alert: Pick<InteractionAlert, 'severity' | 'reviewStatus'>, locale: Locale): string | undefined {
  if (alert.severity !== 'danger' || alert.reviewStatus !== 'pending_medical_review') return undefined;
  return `${t(copy.safety.c2WhatToDoHeading, locale)}: ${t(copy.safety.c2WhatToDoPendingBody, locale)}`;
}
