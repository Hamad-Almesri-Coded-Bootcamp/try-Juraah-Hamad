...
The content, as children. Optionally `onClick` (which turns the whole card into a `<button>` and needs an `aria-label` saying where it goes), `as` to force a different element, `href` together with `as="a"` for a real link, and `flat` to drop the shadow for a card nested inside another surface.
...
Ported against `docs/design-system/components/Card.md` and `bundle.css`'s `.wsf-card` / `.wsf-card--flat` anatomy. `as` resolves to `'button'` when `onClick` is set and to `'div'` otherwise, matching `index.d.ts`; pass `as="a"` with `href` for a link. No prop added, removed or renamed against `index.d.ts`.
