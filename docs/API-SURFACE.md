...
| `savePrescriptionDraft(patientId, draftId)` | Transaction: draft must belong to `S.patient` (**D-014 — refused otherwise; the mock substitutes a placeholder**); insert `prescriptions` (CR-042 source); insert generated `doses` with `tracked` from the patient's settings; delete draft. **AP-10:** after the commit an unflagged one is handed to screening (awaited), or held by a pending `warning` alert when n8n does not accept it (`lib/data/pg/screening.ts`). | `S.patient` | `prescription_drafts`, `prescriptions`, `doses`, `settings` | `prescription_added` |
...
| `requestRefill(patientId, prescriptionId)` | Insert `refill_requests`; trigger `refill_routing` sets `routed_to` from the prescription's own sector and **raises** if the prescription is not the patient's or not `active` (**D-014**). **D10 / CR-082:** after the commit that prescription is screened again, or held. | `S.patient` | `refill_requests`, `prescriptions` | `refill_requested` |
...
These exist because a bot, a calendar client, a browser push service, a cron and the agents track cannot call a server function. **No route handler is reachable from a screen, except the Telegram link route** (a form on A2, E5 and F4 posts to it, because the link token may only ever travel in a server redirect, CR-083 and CR-086), and **no route handler accepts a dose status from a patient, caregiver, reviewer or admin session — there is no such route at all.**
...
| `/api/agent/alerts` | `POST` `InteractionAlert` minus `id`, `reviewStatus` forced to `pending_medical_review` or `auto_cleared` (never `reviewed`), `sourceCitation` required (empty string allowed, `[TO BE SUPPLIED]` allowed) | agent token | Inserts the alert; notifies recipients (§B `alert-recipients` rule) — payloads carry no `actions`. Answers 201 `{ alert, delivered }`. **AP-18/CR-105:** the patient's own Telegram chat is sent to only when `Settings.notificationChannel` is `'telegram'`; `'none'` or any other value sends nothing to the patient. An active caregiver's chat, and push to anyone granted it, are unaffected either way. | `jurah_agent` | `alert_raised` (actor `agent`) | 401 · 403 · 422 (`reviewStatus: 'reviewed'` or any reviewer field) |
| `/api/agent/prescriptions` | `POST` extraction result `{ patientId, prescription: Prescription minus id, needsReview, uncertainFields?, source }` | agent token | Inserts the prescription (CR-042: this is where a real `source` finally arrives) and its doses when confirmed. **CR-090:** after the commit an unflagged one is handed to screening, or held by a pending `warning` alert when n8n does not accept it; 201 `{ prescription, doseCount, screening: 'screened' \| 'held' \| 'skipped' }`, and the caller does not screen a `screened` or `held` one again. | `jurah_agent` | `prescription_added` (actor `agent`); a hold adds `alert_raised` (actor `agent`) | 401 · 403 · 422 (constraint violations, e.g. `doseTimes.length !== frequencyPerDay`) |
...
| `/api/agent/patients/{patientId}/doses?date=` | `GET` | agent token | **CR-062.** That Kuwait date's **tracked** doses of the patient (`id`, `prescriptionId`, `scheduledAt`, `status`, `recordedAt`, generic/brand name, strength, food timing) — an untracked dose is never returned. `date` required, never defaulted. | `jurah_agent` | — | 401 · 403 · 404 · 422 |
| `/api/agent/patients/{patientId}/prescriptions` | `GET` | agent token | **CR-062.** The patient's **active** prescriptions, each with `needsReview` (screening excludes a flagged one visibly, TC-IX-06). | `jurah_agent` | — | 401 · 403 · 404 |
| `/api/agent/patients/{patientId}/voice-turns` | `POST` `{ topic, language, reply }` — `topic` one of `launch`, `next_dose`, `dose_amount`, `today`, `forgot`, `record`, `unclear`, `bye` (`VOICE_TOPICS`); `language` is `ar` or `en`; `reply` non-empty, at most 2000 characters, trimmed; any other key refused | agent token | **CR-069, amended by CR-102.** One Alexa turn, posted by `agent-alexa` after Alexa has already answered. Not clinical data and no dose status. The patient's own open app polls its rows (`lib/assistant` `voiceTurns`, `lib/data/pg/voice.ts`) and moves the page only. Answers 201 `{ id }`. | `jurah_agent` (`INSERT` on five columns of `voice_turns`, migration 0012) | — | 401 no/bad token · 403 any user session · 404 unknown patient · 422 bad body · 503 under the mock backend |
| `/api/agent/demo/reset` | `POST` no body, or `{}`. Any key, or a JSON value that is not an object (array, string, number, null), is refused 422. Text that is not JSON reads as no body, as on every agent route (`readJson`) | agent token | **CR-109, the owner's demo reset** (called only by n8n `agent-demo-reset`, a private Form page outside the app). For `pt-03` only (`DEMO_RESET`, `lib/agent/demo-reset.ts`) and the Kuwait dates today and tomorrow (`kuwaitToday()`): (1) every dose of those dates whose status is not `upcoming` goes back to `upcoming`, `recorded_at` null, `source` `seed` — never to a recorded word, never a delete; (2) every still-`upcoming` `rx-009` dose of those dates at 21:00 Kuwait moves to 19:30, keeping its id. Answers 200 `{ patientId, dates, moved: [{ id, from, to }], reset: [{ id, kuwaitTime, was }] }` — exactly the rows changed, empty arrays when nothing needed doing; a second press answers both empty. | `jurah_agent` for (1) (its column grant on `status`/`recorded_at`/`source`), then `jurah_app` as `system` for (2) (`UPDATE (scheduled_at)`) — two transactions (D-025's grant split) | `dose_status_recorded` (actor `agent`, «… — قادمة: …») once per un-recorded dose, written by the trigger `doses_status_recorded_audit`; the move writes none | 401 no/bad token · 403 any user session · 422 bad body · 503 under the mock backend |
| `/api/messaging/telegram/webhook/{secret}` | `POST` Telegram update | path secret derived from `JURAH_BOT_TOKEN` (never in the repo) | `/start <token>` → find the `pending`, unexpired, unused `messaging_links` row by token; set `connected`, `chat_id`, `connected_at`; consume the token. A used/expired/unknown token → a neutral reply, no write. A token can only ever link *its own* row. **CR-063:** every other update is relayed, after the response, to the agents track (`JURAH_AGENT_INBOUND_URL`, header `x-jurah-secret`) — only from a patient's or an **active** caregiver's connected chat, never written. **CR-092 (AP-05):** the relay payload also carries `trackingOn` (`settings.adherence_check_in_enabled`; `false` when the patient has no settings row), so the agent can say check-ins are off instead of "no dose". | `jurah_app` as `system` | `messaging_connected` | 404 wrong secret · 200 always to Telegram (never leaks) |
| `/api/messaging/telegram/open` | `POST` form `locale` (`ar`\|`en`), `from` (`notifications`\|`setup`\|`profile`) | the signed session cookie; a same-origin post only (the `Origin` host must equal `x-forwarded-host`/`Host`, as Next checks a Server Action; no `Origin`, or an opaque `null` `Origin`, is a refusal; a `Sec-Fetch-Site` header, when the browser sends one, must read `same-origin`) | **AP-09, CR-086.** A patient, or a caregiver whose invitation is `active`: mints the subject's own link through `startMessagingLink` (single use, 15 minutes, one live token per subject) and answers `303` to `https://t.me/<bot>?start=<token>`, the bot's name from Telegram's `getMe` for the configured token (CR-085). The token appears only in that `Location` header. While the bot is simulated (no `JURAH_BOT_TOKEN`) the link is minted and the answer is a `303` back to the screen. `GET` answers `405` (F1's GET route is gone). Every answer: `Cache-Control: no-store`, `Referrer-Policy: no-referrer`. | `jurah_app` as `S.self` (through the seam) | — (a link row, like the seam's own) | `303` back to the screen, no token: no session, a pending-only invitee, a clinic role, a caregiver who is not `active`, a cross-site post, a refused mint, or `getMe` not answering (nothing minted) |
