...
Nine people. Three patients, one caregiver who is also a patient, two invitations that were never accepted, one reviewer, one admin.
...
| 8 | د. خالد عبدالرحمن الرشيد | `280012000961` | reviewer | Works both clinic queues. |
| 9 | م. دانة فهد السالم | `293080700148` | admin | Reads the audit log and nothing else. |
...
Eight records. The first two are the core scenario and every demo starts there.
...
| `rx-008` | سارة | **Levothyroxine** (Eltroxin) 50 mcg | **مستشفى العدان** · *public* | daily | `07:00` | 2026-08-10 | 180 | active |
...
**Settings** — one row per patient, no row for a caregiver.
...
**MessagingLink** — every state present: حمد `not_connected` (the default) · فاطمة `expired` with a dead token · سارة `connected`, linked 2026-08-11 · عبدالله `connected`, linked 2026-09-03 (**alerts only**) · one `pending` row with a live single-use token, belonging to فاطمة's retry. No `chatId` value appears on any screen or in any document, this one included.

**PushSubscription** — every permission present: حمد `default` (never asked) · فاطمة **`denied`** · سارة `granted`, active · عبدالله **`unsupported`** — iPhone Safari with Jur'ah not added to the Home Screen, which is the state that must give install steps rather than a promise. Endpoints and keys are server-side only and appear nowhere.
...
**Caregiver records** — five, one per invitation, covering every state:
...
`name` on each of these is **the name the patient typed**, a claim and not a lookup. `relationship` is "ابني" · "ابنتي" · "ابني" · "زوجة ابني" · "قريب" · "ابنة أختي".

**AuditEvent** — append-only, newest first, at least one row of every type. The rows that matter most:

- every `dose_status_recorded` carries `actor.role: "agent"` (سارة's seven) or `"system"`, and **not one carries `"patient"`, `"caregiver"`, `"reviewer"` or `"admin"`** — filtering the log to this one type is the demo's proof moment
- the full invitation lifecycle: `caregiver_invited` ×6, `caregiver_invite_accepted` ×2, `caregiver_invite_declined` ×1, `caregiver_invite_expired` ×1, plus a `caregiver_invite_cancelled` and a `caregiver_revoked` from an older invitation kept for the log's sake
