...
2. **Test list.** The ID must be one of the **twelve** Civil IDs in `Seed Dataset.md` (the eleven `Account` rows plus `277091900873`, which has no account). Any other value → `not_in_test_list`, with the plain error `identity.invalidIdError` ("This number can't be used to sign in here", reworded by CR-107 in docs/DECISIONS.md), input kept. This is the **only** rejection A1 makes, and it reveals nothing about accounts because every account-holding ID in the demo is on the list by construction.
