...
| Travel check said `no_interaction_found` when a drug wasn't covered | New verdict `cannot_verify` - also for an ungraded pair, or while any prescription awaits review. Its own app outcome kind, `cannot_verify` (D6/CR-078, AP-11); `needs_confirmation` and `could_not_identify` still map to the app's `could_not_identify` | "Not in our data" is not "no interaction", and reads differently from "we could not read the box" |
