...
test('trustFreeTalk: only a listed intent above 0.7; record keeps only well-formed items; record with none is unclear', () => {
  ...
  const r = A.trustFreeTalk({ intent: 'record', confidence: 0.95, items: [
    { position: 1, status: 'taken_on_time' }, { position: 2, status: 'raise_dose' }, { position: 99, status: 'missed' }, { time: '21:00', status: 'missed' }, {},
  ] });
  assert.equal(r.kind, 'record');
  assert.deepEqual(r.items, [{ position: 1, time: null, medicine: null, status: 'taken_on_time' }, { position: null, time: '21:00', medicine: null, status: 'missed' }]);
  assert.deepEqual(A.trustFreeTalk({ intent: 'record', confidence: 0.95, items: [{ status: 'missed' }] }), { kind: 'AMAZON.FallbackIntent', items: [] });
...
test('"mark the first two taken and the third missed" at 22:00: positions count today in time order; read back, nothing written', () => {
  const r = plan([{ position: 1, status: 'taken_on_time' }, { position: 2, status: 'taken_on_time' }, { position: 3, status: 'missed' }], '22:00');
  assert.deepEqual(r.pending, [
    { doseId: 'rx-008-0924-0700', prescriptionId: 'rx-008', status: 'taken_on_time' },
    { doseId: 'rx-009-0924-1300', prescriptionId: 'rx-009', status: 'taken_on_time' },
    { doseId: 'rx-009-0924-2100', prescriptionId: 'rx-009', status: 'missed' },
  ]);
...
test('a dose not due yet (more than an hour ahead) or already recorded is named and skipped - never written', () => {
  const r = plan([{ position: 1, status: 'taken_on_time' }, { position: 2, status: 'taken_on_time' }, { position: 3, status: 'missed' }], '09:00');
  ...
  const r2 = plan([{ position: 1, status: 'missed' }], '09:00', 'en', done);
...
test('by medicine name and by time; "my calcium" with two calcium doses picks the one that is open and due, else refuses as ambiguous', () => {
  assert.deepEqual(plan([{ medicine: 'eltroxin', status: 'taken_late' }], '09:00').pending.map((p) => [p.doseId, p.status]), [['rx-008-0924-0700', 'taken_late']]);
  assert.deepEqual(plan([{ time: '13:00', status: 'taken_on_time' }], '13:10').pending.map((p) => p.doseId), ['rx-009-0924-1300']);
  const doneMorning = DAY(); doneMorning[2] = dose('rx-009-0924-1300', 'rx-009', '13:00', 'taken_on_time');
  assert.deepEqual(plan([{ medicine: 'calcium', status: 'missed' }], '22:00', 'en', doneMorning).pending.map((p) => p.doseId), ['rx-009-0924-2100']);
  assert.match(plan([{ medicine: 'calcium', status: 'taken_on_time' }], '22:00').speech, /more than one dose matches/);
  assert.match(plan([{ medicine: 'aspirin', status: 'taken_on_time' }], '22:00').speech, /could not find the dose you meant/);
});

test('Arabic wording, and the schedule could not be read -> no speech here (the caller says it failed), nothing pending', () => {
  assert.match(plan([{ position: 1, status: 'taken_on_time' }], '09:00', 'ar').speech, /^بسجّل: Eltroxin الساعة 7 الصبح أخذتها\. تأكد؟ قول نعم، أو لا\.$/);
  assert.deepEqual(A.planRecord({ items: [{ position: 1, status: 'missed' }], doses: null, nowIso: at('09:00'), language: 'en', spokenTime: V.spokenTime }), { speech: null, pending: [] });
});
...
test('"yes": the pending list from Alexa\'s session is re-checked against FRESH doses; writes are the Telegram path\'s own two calls', () => {
  const pending = [
    { doseId: 'rx-008-0924-0700', prescriptionId: 'rx-008', status: 'taken_on_time' },
    { doseId: 'rx-009-0924-2100', prescriptionId: 'rx-009', status: 'missed' },
    { doseId: 'rx-009-0924-1300', prescriptionId: 'rx-008', status: 'taken_on_time' }, // prescription does not match -> skipped
    { doseId: '../../etc', prescriptionId: 'rx-008', status: 'taken_on_time' }, // malformed -> dropped
  ...
  assert.deepEqual(r.writes.map((w) => [w.url, w.body, w.recompute]), [
    [API + '/doses/rx-008-0924-0700/status', { status: 'taken_on_time', recordedAt: '2026-09-24T19:00:00.000Z', source: 'adherence_agent' }, null],
    [API + '/doses/rx-009-0924-2100/status', { status: 'missed', recordedAt: '2026-09-24T19:00:00.000Z', source: 'adherence_agent' },
...
test('after the writes: says what was recorded and, honestly, what was not', () => {
  const writes = [{ doseId: 'rx-008-0924-0700', status: 'taken_on_time' }, { doseId: 'rx-009-0924-2100', status: 'missed' }];
